...
let apiDataProducts;
displayBasket(apiDataProducts);

// Récuéprer les informations de l'api
async function callApi(){
  apiDataProducts = await fetch("http://localhost:3000/api/products/")
  ...
  for (productOrdered in basket) {
    ...
    newArticle.innerHTML = `
      ...
      <input type="number" id="${
        basket[productOrdered].colorOfProduct + basket[productOrdered].idOfProduct
      ...
      <p class="deleteItem" id="${
        basket[productOrdered].idOfProduct + basket[productOrdered].colorOfProduct
      }">Supprimer</p>
      </div>
      </div>
      </div>
...
// Parcourir le array basket que nous venons de recuperer et créer les balises html.

// function browse(basket, apiDataProduct) {
